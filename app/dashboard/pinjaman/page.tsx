...
import { MemberModeSync } from "../simpanan/MemberModeSync"; // Menggunakan ulang dari modul simpanan!
...
// CSS diekstrak agar aman dari Vercel compiler
const pageStyles = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  /* ── Filter Chips ── */
  .kop-filter-row { display: flex; gap: 10px; overflow-x: auto; padding-bottom: 8px; margin-bottom: 16px; -ms-overflow-style: none; scrollbar-width: none; }
  .kop-filter-row::-webkit-scrollbar { display: none; }
  .kop-filter-chip {
    display: inline-flex; align-items: center; padding: 10px 16px; border-radius: 14px;
    font-size: 13px; font-weight: 700; text-decoration: none; white-space: nowrap; transition: all 0.2s ease;
  }
  .kop-filter-chip:active { transform: scale(0.96); }

  /* ── Pinjaman Card ── */
  .kop-pinjaman-card {
    background: #fff; border-radius: 16px; border: 1.5px solid #f1f5f9;
    box-shadow: 0 4px 12px rgba(15,45,107,.04); padding: 20px; text-decoration: none;
  ...
  @media (max-width: 768px) {
    .kop-stats-grid { grid-template-columns: repeat(2, 1fr); gap: 12px; }
  }
...
export default async function PinjamanPage({
  ...
  // Logika RBAC yang digabung dengan Member Mode Sync
  ...
  // Menentukan View: Jika Pengurus dan TIDAK ada parameter ?view=personal, maka tampilkan mode Admin
  const isPengurusView = roleIsPengurus && viewParam !== "personal";

  // Fetch Data Sesuai View
  const { data: pinjamanList } = await getPinjamanList({
    ...(filterStatus ? { status: filterStatus as any } : {}),
    ...(!isPengurusView ? { userId: currentUser.id } : {}), // Hanya ambil milik sendiri jika bukan Admin
  });
  
  // Statistik hanya diambil jika di tampilan pengurus
  ...
  // Helper untuk navigasi internal (mempertahankan ?view=personal jika ada)
  ...
  return (
    <main className="kop-shell bg-slate-100 min-h-screen">
      {/* Menggunakan ulang MemberModeSync dari modul simpanan */}
        ...
        <div className="kop-content-wrapper">
          ...
          {isPengurusView && stats && (
            <div className="kop-stats-grid">
              <div className="kop-card" style={{ padding: "20px", textAlign: "center", margin: 0 }}>
                ...
                <div style={{ fontSize: "11px", color: "#64748b", marginTop: "4px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  Total Pengajuan
                </div>
              </div>
              <div className="kop-card" style={{ padding: "20px", textAlign: "center", margin: 0 }}>
              ...
              <div className="kop-card" style={{ padding: "20px", textAlign: "center", margin: 0 }}>
                <div style={{ fontSize: "18px", fontWeight: "900", color: "#1d4ed8", marginTop: "8px" }}>
          ...
          <div className="kop-filter-row">
            {filters.map((f) => {
              ...
              return (
                <Link
                  key={f.label}
                  href={getFilterUrl(f.value)}
                  className="kop-filter-chip"
                  style={{
          ...
          {pinjamanList.length === 0 ? (
            <div className="kop-card" style={{ textAlign: "center", padding: "64px 20px" }}>
              <svg style={{ margin: "0 auto 16px auto", display: "block", color: "#cbd5e1" }} width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <line x1="12" y1="1" x2="12" y2="23" />
                <path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" />
              </svg>
              ...
              {!isPengurusView && (
                <Link href="/dashboard/pinjaman/ajukan?view=personal" style={{ background: "#f1f5f9", color: "#1d4ed8", padding: "10px 20px", borderRadius: "12px", fontSize: "13px", fontWeight: "700", marginTop: "20px", display: "inline-block", textDecoration: "none" }}>
                ...
                return (
                  <Link key={p.id} href={`/dashboard/pinjaman/${p.id}${!isPengurusView ? '?view=personal' : ''}`} className="kop-pinjaman-card">
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
                      ...
                      <span
                        style={{
                          background: statusInfo.bg, color: statusInfo.color, border: statusInfo.border,
