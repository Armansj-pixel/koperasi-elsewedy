...
export default function NOCCommandCenterPage() {
  ...
  const [isSyncing, setIsSyncing] = useState(false) // State untuk efek tombol
  ...
  // Fungsi khusus untuk tombol Force Resync
  const handleForceResync = async () => {
    setIsSyncing(true)
    await fetchHealth()
    // Tambahan delay sedikit agar efek visual "Scanning" terlihat oleh user
    setTimeout(() => {
      setIsSyncing(false)
    }, 1200) 
  }
  ...
  return (
    <div className="noc-wrapper min-h-screen w-full p-3 md:p-6 flex flex-col relative overflow-hidden selection:bg-[#00ff41] selection:text-black">
        ...
        <div className="flex flex-wrap justify-center lg:justify-end gap-4 md:gap-6 font-mono-tech text-[10px] md:text-sm">
          <div className="text-center lg:text-right">
            ...
            <div className="text-lg md:text-xl glow-text">
                {isSyncing ? 'SYNCING' : (data ? 'ONLINE' : 'CONNECTING')} 
                <i className="inline-block w-1.5 h-1.5 bg-[#00ff41] rounded-full cursor-blink align-middle ml-1"></i>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 flex-grow z-10 w-full">
        ...
        {/* PANEL KIRI: METRICS RESOURCES LENGKAP */}
        <div className="lg:col-span-3 flex flex-col gap-4">
          <div className="cyber-panel p-4 flex-grow flex flex-col">
            <h2 className="text-[10px] md:text-xs tracking-widest border-b border-emerald-900/50 pb-2 mb-4 font-bold uppercase glow-text text-white">Service Endpoints & Modules</h2>
            
            <div className="flex-grow flex flex-col justify-center space-y-4 font-mono-tech">
              {/* Endpoint Status */}
              <div>
                <div className="flex justify-between text-[10px] text-emerald-500 mb-1"><span>DB_SUPABASE (POSTGRES)</span> <span>{data?.metrics.db_latency || '--ms'}</span></div>
                <div className="led-bar">
                  <div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div>
                  <div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div><div className="led-segment"></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between text-[10px] text-emerald-500 mb-1"><span>AUTH_SUPABASE (GoTrue)</span> <span>15ms</span></div>
                <div className="led-bar">
                  <div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div>
                  <div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div>
                </div>
              ...
              <div>
                <div className="flex justify-between text-[10px] text-emerald-500 mb-1"><span>STORAGE_CLOUDINARY</span> <span className="text-white">42ms</span></div>
                <div className="led-bar">
                  <div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div><div className={`led-segment ${data ? 'active' : ''}`}></div>
                  <div className={`led-segment ${data ? 'active' : ''}`}></div><div className="led-segment"></div><div className="led-segment"></div>
                </div>
              </div>
              <div>
                <div className="flex justify-between text-[10px] text-emerald-500 mb-1"><span>SMTP_GATEWAY (RESEND)</span> <span className="text-orange-500">145ms</span></div>
                <div className="led-bar">
          ...
          <div className="cyber-panel p-4 font-mono-tech">
            <h2 className="text-[10px] md:text-xs tracking-widest border-b border-emerald-900/50 pb-2 mb-3 font-bold uppercase glow-text text-white">Core Modules Integrity</h2>
            <ul className="text-[10px] space-y-2">
                <li className="flex justify-between items-center border-b border-emerald-900/30 pb-1">
                    <span className="text-emerald-600">MODUL_SIMPANAN</span>
                    <span className="text-white font-bold bg-emerald-900/40 px-2 py-0.5 rounded">ONLINE</span>
                </li>
                <li className="flex justify-between items-center border-b border-emerald-900/30 pb-1">
                    <span className="text-emerald-600">MODUL_PINJAMAN</span>
                    <span className="text-white font-bold bg-emerald-900/40 px-2 py-0.5 rounded">ONLINE</span>
                </li>
                <li className="flex justify-between items-center border-b border-emerald-900/30 pb-1">
                    <span className="text-emerald-600">CRON_JOB_ENGINE</span>
                    <span className="text-white font-bold bg-emerald-900/40 px-2 py-0.5 rounded">IDLE / OK</span>
                </li>
            </ul>
          </div>
        </div>

        {/* PANEL TENGAH: REAL TERMINAL AUDIT LOG */}
        <div className="md:col-span-2 lg:col-span-6 flex flex-col gap-4">
          <div className="cyber-panel flex-grow p-1 flex flex-col h-[350px] lg:h-auto">
            <div className="bg-emerald-900/30 border-b border-emerald-900/50 p-2 flex justify-between items-center font-mono-tech text-[10px]">
              ...
              <span className="text-emerald-500 truncate ml-2">root@elsewedy-sys:~ # {isSyncing ? 'fetching audit trails...' : 'tail -f /var/log/audit.log'}</span>
            </div>
            
            <div className="p-3 font-mono-tech text-[10px] md:text-xs overflow-y-auto flex-grow flex flex-col justify-end space-y-1.5 break-words bg-black/40 relative">
              
              {/* Overlay Matrix Loading Effect saat tombol diklik */}
              {isSyncing && (
                <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-20 text-emerald-500">
                    <i className="fa-solid fa-circle-notch fa-spin text-3xl mb-2 glow-text"></i>
                    <span className="animate-pulse tracking-widest">EXECUTING SECURE DIAGNOSTICS...</span>
                </div>
              )}
        ...
        {/* PANEL KANAN: INFRASTRUCTURE & SECURITY */}
        <div className="md:col-span-2 lg:col-span-3 flex flex-col gap-4">
          
          <div className="cyber-panel p-4 font-mono-tech">
            <h2 className="text-[10px] md:text-xs tracking-widest border-b border-emerald-900/50 pb-2 mb-3 font-bold uppercase glow-text text-white">Hardware Resources</h2>
            <div className="grid grid-cols-2 gap-3 text-center">
              ...
              <div className="border border-emerald-900/50 p-2 bg-emerald-950/20">
                ...
                <div className="text-xl mt-1 glow-text-cyan">18<span className="text-[10px] text-emerald-700">%</span></div>
              </div>
              <div className="border border-emerald-900/50 p-2 bg-emerald-950/20">
                <div className="text-[9px] text-emerald-600">CONNECTION_POOL</div>
                <div className="text-xl mt-1 text-white font-bold">2/15</div>
              </div>
            </div>
          </div>
          ...
          <div className="cyber-panel p-4 border-l-2 border-l-yellow-500 flex-grow flex flex-col">
            ...
            {/* TOMBOL FORCE RESYNC DENGAN STATE LOADING */}
            <button 
              onClick={handleForceResync}
              disabled={isSyncing}
              className={`mt-4 w-full border font-mono-tech py-2 text-[10px] md:text-xs font-bold tracking-widest transition-all duration-300 ${
                isSyncing 
                  ? 'border-emerald-500 bg-emerald-500 text-black cursor-wait shadow-[0_0_15px_#00ff41]' 
                  : 'border-emerald-500/50 bg-emerald-900/20 hover:bg-emerald-500 hover:text-black text-emerald-500 active:scale-[0.98]'
              }`}>
              {isSyncing ? '[ EXECUTING DIAGNOSTICS... ]' : '[ FORCE SECURITY RESYNC ]'}
            </button>
          </div>
        </div>
      </div>
