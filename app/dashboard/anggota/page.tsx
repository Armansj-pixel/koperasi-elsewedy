...
export default async function AnggotaPage({
  ...
  const totalAktif = anggotaList.filter(
    (a: any) => a.is_active
  ).length;
  const totalNonaktif = anggotaList.filter(
    (a: any) => !a.is_active
  ).length;
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 px-4 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          {["SUPERADMIN", "SEKRETARIS"].includes(
            currentUser.role
          ) && (
            <Link
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-slate-400">
                          {anggota.email?.includes("@koperasi.local")
                            ? "-"
                            : anggota.email}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 hidden md:table-cell text-right font-bold text-green-700">
                        Rp{" "}
                        {Number(
