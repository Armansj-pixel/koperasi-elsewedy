...
import { topUpKasKecil, catatPengeluaranOperasional } from "@/lib/akuntansi/actions";

export default function KasForm({ expenseAccounts }: { expenseAccounts: any[] }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [activeTab, setActiveTab] = useState<"KAS" | "BIAYA">("BIAYA");

  const [notif, setNotif] = useState<{ type: "success" | "error"; msg: string } | null>(null);

  // Form Pengeluaran Biaya
  async function handleCatatBiaya(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        setNotif({ type: "success", msg: "✅ Biaya berhasil dicatat ke Buku Besar!" });
        (e.target as HTMLFormElement).reset();
        router.refresh();
      } else {
        setNotif({ type: "error", msg: res.error || "Gagal mencatat biaya." });
      }
    });
  }

  // Form Isi Kas Laci (Tarik Tunai)
  async function handleTopUpKas(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        setNotif({ type: "success", msg: "✅ Tarik Tunai berhasil dicatat ke Buku Besar!" });
        (e.target as HTMLFormElement).reset();
        router.refresh();
      } else {
        setNotif({ type: "error", msg: res.error || "Gagal top-up kas." });
      }
    });
  }
    ...
    <div style={{ background: "#fff", borderRadius: "20px", border: "1px solid #e2e8f0", boxShadow: "0 10px 30px rgba(0,0,0,0.03)", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", borderBottom: "1.5px solid #f1f5f9" }}>
        <button 
          ...
          style={{ flex: 1, padding: "16px", fontSize: "14px", fontWeight: 800, background: activeTab === "BIAYA" ? "#fff" : "#f8fafc", color: activeTab === "BIAYA" ? "#0f766e" : "#64748b", border: "none", borderBottom: activeTab === "BIAYA" ? "3px solid #0f766e" : "3px solid transparent", cursor: "pointer", transition: "all 0.2s" }}
        >
          Catat Biaya Operasional
        </button>
        <button 
          onClick={() => setActiveTab("KAS")}
          style={{ flex: 1, padding: "16px", fontSize: "14px", fontWeight: 800, background: activeTab === "KAS" ? "#fff" : "#f8fafc", color: activeTab === "KAS" ? "#0f766e" : "#64748b", border: "none", borderBottom: activeTab === "KAS" ? "3px solid #0f766e" : "3px solid transparent", cursor: "pointer", transition: "all 0.2s" }}
        >
          Tarik Tunai (Isi Laci Kas)
        </button>
      </div>
      ...
      <div style={{ padding: "24px" }}>
              ...
              <div>
                ...
                <select name="akun_biaya" required style={inputStyle}>
                  ...
                  {expenseAccounts.map(a => (
                    <option key={a.id} value={a.kode_akun}>{a.kode_akun} - {a.nama_akun}</option>
                  ))}
                </select>
              </div>
              <div>
        ...
        {/* TAB 2: TARIK TUNAI KAS */}
        {activeTab === "KAS" && (
          <form onSubmit={handleTopUpKas} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div style={{ padding: "16px", background: "#f0fdf4", border: "1px dashed #86efac", borderRadius: "12px", fontSize: "13px", color: "#15803d", fontWeight: 600, lineHeight: 1.5 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              ...
              <div>
                ...
                <input type="number" name="nominal" min="1000" required placeholder="Contoh: 1000000" style={{...inputStyle, fontSize: "16px", fontWeight: 800, color: "#1d4ed8"}} />
              </div>
            </div>
            ...
            <button type="submit" disabled={isPending} style={{ marginTop: "8px", background: "#1d4ed8", color: "#fff", border: "none", padding: "14px", borderRadius: "12px", fontSize: "14px", fontWeight: 800, cursor: "pointer", opacity: isPending ? 0.7 : 1 }}>
